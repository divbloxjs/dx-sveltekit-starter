...
;
	...
	// 	isDefault:
	// 		$page?.data?.userSession?.isDefault?.toString() ??
	// 		$page?.form?.isDefault?.toString() ??
	// 		'null',

	// 	// loop over all relationships and add as below
	// 	placeId:
	// 		$page?.data?.userSession?.placeId?.toString() ?? $page?.form?.placeId?.toString() ?? 'null'
	// };
</script>

<a href="/userSession/overview">back</a>
<form method="POST" action="/userSession/{formValues.id}?/update" use:enhance>
	...
	<button formaction="/userSession/{formValues.id}?/delete">Delete</button>
</form>

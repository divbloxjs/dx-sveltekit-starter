<script>
    ...
    import dataTableConfig from "./data-series/task-data-table.config.json";
    ...
    $: (() => {
        flatRows = [];
        for (const nestedRow of data.taskArray) {
            ...
            <tr class="odd:bg-background-100 hover:bg-background-200 child:p-2">
                ...
                {#if allowEdit || allowDelete}
                    <td class="flex items-center justify-center text-center">
                        <a
                        href={`${basePath}/${data?.taskArray[index]?.id}`}
                            class="bg-tranparent border-tertiary text-tertiary border border-none">
                            <Pencil class="h-4 w-4" /></a>

                    <form action={`${basePath}/${data?.taskArray[index]?.id}?/delete`} use:enhance method="POST">
                        <input type="hidden" bind:value={data.taskArray[index].id} />

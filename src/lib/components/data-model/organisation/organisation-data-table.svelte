<script>
    ...
    import dataTableConfig from "./data-series/organisation-data-table.config.json";
    ...
    let attributeMap = {};
    buildAttributeMap("organisation", dataTableConfig, attributeMap);

    let flatRows = [];
    $: (() => {
        flatRows = [];
        for (const nestedRow of data.organisationArray) {
            ...
            <tr class="odd:bg-background-100 hover:bg-background-200 child:p-2">
                ...
                {#if allowEdit || allowDelete}
                    <td class="flex items-center justify-center text-center">
                        <a
                            href={`${basePath}/${data?.organisationArray[index]?.id}`}
                            class="bg-tranparent border border-none border-tertiary text-tertiary">
                            <Pencil class="h-4 w-4" />
                        </a>

                        <form action={`${basePath}/${data?.organisationArray[index]?.id}?/delete`} use:enhance method="POST">
                            <input type="hidden" bind:value={data.organisationArray[index].id} />

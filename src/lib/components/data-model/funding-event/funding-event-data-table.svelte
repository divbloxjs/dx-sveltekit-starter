<script>
    ...
    import dataTableConfig from "./data-series/funding-event-data-table.config.json";
    ...
    import FundingEventDataTableRow from "./data-series/funding-event-data-table-row.svelte";
    ...
    let attributeMap = {};
    buildAttributeMap("fundingEvent", dataTableConfig, attributeMap);

    let flatRows = [];
    $: (() => {
        flatRows = [];
        for (const nestedRow of data.fundingEventArray) {
...
<div class="w-full overflow-x-auto">
    <table class="w-full table-auto border">
        ...
        {#each flatRows as flatRow, index}
            <FundingEventDataTableRow {flatRow} rowId={data?.fundingEventArray[index]?.id} {basePath} {allowEdit} {allowDelete}/>
        {/each}
    </table>
</div>
